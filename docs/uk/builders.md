...
### Усередині адреси вирішує сама наявність

У `changeInternationalAddress()` та `changeLocalizedAddress()` те, чи ключ **присутній**, і є всією
інструкцією:

| Ви пишете | Що станеться |
|---|---|
| ключа немає | поле не надсилається, і реєстр лишає значення, яке тримає |
| ключ містить значення | полю задається це значення |
| ключ містить `''` | поле надсилається порожнім, що його **очищає** |

Порожній рядок — це єдиний спосіб прибрати необов'язкове поле: `org`, `stateProvince` чи
`postalCode`. Іншого написання для «видали це» немає.

**Блок адреси — це послідовність з обов'язковими містом і країною**, тож він виводиться або цілком,
або ніяк. Торкнетеся будь-якої його частини — `street`, `city`, `stateProvince`, `postalCode`,
`countryCode` — і подавайте `city` та `countryCode` у тому самому виклику. Якщо їх не подати, вони
підуть порожніми елементами, а це очистить місто контакту, якому ви хотіли лише змінити номер
будинку.

Зміна однієї форми ніколи не зачіпає другої.
...
```js
// Клієнт переїхав і відмовився від назви компанії. Форма локальним письмом і все інше в цьому
// контакті лишаються рівно такими, як були.
await client.contact.updateBuilder('C1')
  .changeInternationalAddress({
    street: ['vul. Svobody 1'],
    city: 'Lviv',
    countryCode: 'UA',
    org: '',                       // '' ОЧИЩАЄ його; якби ключа не було, він би лишився
  })
