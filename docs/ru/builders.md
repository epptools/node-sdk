...
`changeInternationalAddress()` и `changeLocalizedAddress()` передают реестру блок, который
**замещает** тот, что реестр хранит. Их не сливают поле за полем, поэтому всё, чего вы не передали,
исчезает:
...
замещает блок, — причём **каждая из тех команд отвечает 1000** — блок, отправленный без `org`,
...
**Сначала прочитайте блок и верните его обратно вместе со своим изменением.**

Изменение одной формы никогда не трогает другую: `int` и `loc` адресуются раздельно.
...
```js
// Клиент переехал и отказался от названия компании. Сначала читаем блок: всё, чего вы не
// передадите, будет удалено, поэтому неизменные части обязаны поехать вместе с изменением.
const current = (await client.contact.info('C1')).postalInfo().int;

await client.contact.updateBuilder('C1')
  .changeInternationalAddress({
    ...current,                    // name и всё остальное, что хранит реестр
    street: ['vul. Svobody 1'],
    city: 'Lviv',
    countryCode: 'UA',
    org: '',                       // '' ОЧИЩАЕТ его — и здесь его очистило бы также отсутствие
  })
