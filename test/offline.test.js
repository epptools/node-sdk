...
async function main() {
...

  {
    ...
    // RFC 8590. A notice about something the registry did to your object arrives as a sentence in
    // the language of your account, which no program can rely on; this is the same event as data.
    const chgRes = Response.fromXml(
      '<?xml version="1.0"?><epp xmlns="urn:ietf:params:xml:ns:epp-1.0"><response>' +
      '<result code="1301"><msg>Command completed successfully; ack to dequeue</msg></result>' +
    ...
    check('a change notice says what happened, without reading the sentence',
      chgRes.change().operation === 'delete' && chgRes.change().who === 'Registry'
      && chgRes.change().reason === 'deleted');
    // Which way resData reads. A domain that no longer exists can only be described as it last was,
    // and treating that as its CURRENT state is how a client resurrects a deleted object.
    check('and which way the object beside it reads',
      chgRes.change().state === 'before' && chgRes.objectName() === 'gone.com.ua');

    const chgDefault = Response.fromXml(infData(
    ...
    check('an omitted state means after, and a custom operation keeps its own verb',
      chgDefault.change().state === 'after' && chgDefault.change().op === 'sync'
